...
$standardsprache = getVermieterEigenschaftenWert(STANDARDSPRACHE,$vermieter_id);
	...
	?>
		<p class="<?php echo UEBERSCHRIFT ?>"><?php echo(getUebersetzung("Ändern der Buchungsbestätigung")); ?></p>
	<?php
	}
	else if (isset($_POST[BUCHUNGS_ABLEHNUNG])){
		$art = BUCHUNGS_ABLEHNUNG;
	?>
		<p class="<?php echo UEBERSCHRIFT ?>"><?php echo(getUebersetzung("Ändern des Absagetextes einer Anfrage")); ?></p>
	<?php
	}
	else if (isset($_POST[ANFRAGE_BESTAETIGUNG])){
		$art = ANFRAGE_BESTAETIGUNG;
	?>
		<p class="<?php echo UEBERSCHRIFT ?>"><?php echo(getUebersetzung("Ändern des Bestätigungstextes einer Buchungsanfrage")); ?></p>
	<?php
	}
