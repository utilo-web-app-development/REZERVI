...
<form action="./sendEMail.php" method="post" name="e_mail_form" target="_self" id="e_mail_form">
  <table border="0" cellspacing="0" cellpadding="3">
    <tr class="<?= STANDARD_SCHRIFT ?>">
      ...
      <td><input name="name" type="text"  id="name" size="50" <? if (isset($name)) echo("value=\"$name\""); ?>/>*</td>
    </tr>
    <tr class="<?= STANDARD_SCHRIFT ?>">
      <td><?php echo(getUebersetzung("E-Mail-Adresse")); ?></td>
      <td><input name="email" type="text"  id="email" size="50" <? if (isset($email)) echo("value=\"$email\""); ?>/>*</td>
    </tr>
    <tr class="<?= STANDARD_SCHRIFT ?>">
      <td><?php echo(getUebersetzung("Telefonnummer")); ?></td>
      <td><input name="telefon" type="text"  id="telefon" size="50" <? if (isset($telefon)) echo("value=\"$telefon\""); ?>/></td>
    </tr>
    <tr class="<?= STANDARD_SCHRIFT ?>">
  ...
  <p>
    <input name="send" type="submit" class="<?= BUTTON ?>" onMouseOver="this.className='<?= BUTTON_HOVER ?>';"
       onMouseOut="this.className='<?= BUTTON ?>';" id="send" value="<?php echo(getUebersetzung("Absenden")); ?>"/>
