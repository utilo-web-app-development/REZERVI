...
var maerz= '<?php echo getUebersetzung("März") ?>';
...
function DateInput(DateName, Required, DateFormat, DefaultDate) {
   if (DateName == undefined) document.writeln('<span style="color:red;font-size:' + FontSize + 'px;font-family:' + FontFamily + ';">ERROR: Missing required parameter in call to \'DateInput\': [name of hidden date field].</span>');
      ...
      with (document) {
         ...
         writeln('<table cellpadding="0" cellspacing="2"><tr>' + String.fromCharCode(13) + '<td>');
         //bildchen:
         ...
         //tag:
         writeln('<select class="<?php echo SELECT ?>"' + InitialStatus + '  id="' + DateName + '_Day_ID" name="' + DateName + '_Day_ID" onChange="' + DateName + '_Object.changeDay(this)">');
         for (var j=1;j<=eval(DateName + '_Object.picked.dayCount');j++) {
            DaySelected = ((DefaultDate != undefined) && eval(DateName + '_Object.picked.day=='+j)) ? ' selected' : '';
            writeln('<option' + DaySelected + '>' + j + '</option>');
         }
         writeln('</select>' + String.fromCharCode(13) + '</td>' + String.fromCharCode(13) + '<td>');
             
         //monat:
         writeln('<select class="<?php echo SELECT ?>" name="' + DateName + '_Month"  onChange="' + DateName + '_Object.changeMonth(this)">');
         if (!Required) {
