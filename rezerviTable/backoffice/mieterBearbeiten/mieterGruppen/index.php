<?php
$root = "../../.."; 
$ueberschrift = "Gästegruppen bearbeiten";
...
if(isset($_POST["aendern"])){
	$result = updateGaesteGruppe($name, $gruppenname, $beschreibung, $status, $gastro_id);
	if($result){
		$info = true;
...
if(isset($_POST["anlegen"])){
	$result = insertGaesteGruppe($name, $beschreibung, $status, $gastro_id);
	if($result){
		$info = true;
...
if(isset($_POST["loeschen"])){
	if($gruppenname == $name){
		deleteGaesteGruppe($gruppenname, $gastro_id);
		$info = true;
...
if($gruppenname != ""){
	$name = $gruppenname;
	$beschreibung = getBeschreibungOfGaesteGruppe($gruppenname, $gastro_id);
	$status = getStatusOfGaesteGruppe($gruppenname, $gastro_id);
}else{
...
$breadcrumps = erzeugenBC($root, "Gäste", "mieterBearbeiten/index.php",
								$unterschrift, "mieterBearbeiten/mieterGruppen/index.php");
include_once($root."/backoffice/templates/bodyStart.inc.php"); 
?>
<table>
    <h2><?php echo(getUebersetzung("Gästegruppen ändern/anlegen/löschen")); ?></h2>
